...
- Seeded login (password `SkillServe#2026`): `admin@skillserve.test` (super-admin) — the **only** account seeding creates. Further staff accounts are made by hand in Administrator Management. Mobile `customer@skillserve.test` and `provider@skillserve.test` come from the demo seed only (`DemoAccountSeeder`).
