---
type: index
tags: [index, domain]
---
# Domain & Business Logic Index

Entities and rules as implemented in the backend services (the single authoritative place for
business rules — AGENT.md).

## Model
- [[Domain Model Overview]] — entities and how they relate
- [[User Types and Roles]] — account kinds (in 00)

## Lifecycles (state machines)
- [[Booking Lifecycle]]
- [[Cancellation and Fees]]
- [[Payments and Refunds]]
- [[Disputes Lifecycle]]
- [[Provider Verification Lifecycle]]
- [[Service Approval Lifecycle]]
- [[Reports and Moderation Lifecycle]]
- [[Account Status Lifecycle]]
- [[Support Ticket Lifecycle]]
- [[Registration and OTP Flow]]

## Rules & catalogs
- [[Reviews and Ratings Rules]]
- [[Notifications Catalog]]
- [[System Settings Catalog]]
- [[Time and Timezone Rules]]
- [[Data Retention and Deletion]]
- [[Marketplace Visibility Rules]]

Back to [[Home]]
